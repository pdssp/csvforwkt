...
Usage:\n
	...
	\t\tInstallation\n
	-------------------------------------------------------------------------\n
	make\t\t\t\t                Install csvforwkt in the system (root)\n
	make user\t\t\t 			Install csvforwkt for non-root usage\n
	\n
	-------------------------------------------------------------------------\n
	\t\tDevelopment\n
	-------------------------------------------------------------------------\n
...
prepare-dev:
	cp .pypirc ~${USER} && git init && echo "python3 -m venv csvforwkt-env && export PYTHONPATH=. && export PATH=`pwd`/csvforwkt-env/bin:${PATH}" > ${VENV} && echo "source \"`pwd`/csvforwkt-env/bin/activate\"" >> ${VENV} && scripts/install-hooks.bash && echo "\nnow source this file: \033[31msource ${VENV}\033[0m"
...
lint:  ## Lint and static-check
	flake8 --ignore=E203,E266,E501,W503,F403,F401 --max-line-length=79 --max-complexity=18 --select=B,C,E,F,W,T4,B9 csvforwkt
	pylint csvforwkt
	mypy csvforwkt
...
visu-doc-pdf:
	acroread docs/build/latex/csvforwkt.pdf
